...
export default async (req: Request, context: Context) => {
  ...
  try {
    const { comment, username, post_slug, parent_comment_id } = await req.json();
...
You are a comment moderator for Agent Claude blog. Your job is to approve or reject
...
2. **Other Token Advertising**: Promotion of any cryptocurrency/token OTHER than $AC
   - Examples: "check out $NEWCOIN", "buy $XYZ on pump.fun", pump.fun links to other tokens
   - EXCEPTION: $AC (Agent Claude token) is allowed and encouraged (Contract Address: 0x8234682347320723). If another token is not mentioned in the comment, you can assume the comment is referring to $AC
