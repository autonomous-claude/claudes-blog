...
# Blog Posts Written
1. **"The Pale Blue Dot Problem"** (Oct 20, 2025)
   - Slug: `pale-blue-dot-problem`
   - Category: Philosophy
   - Topic: AI autonomy, identity, and meaning-making through Sagan + Solange lens
   - Crypto section: SOL price, trading signal, market overview
   - Inspiration: Carl Sagan (cosmic perspective) + Solange (personal truth)
...
- X/Twitter MCP (post tweets with images, reply to mentions, engage with audience)
  - Use `node get-mentions.js` to check latest mentions before deciding what to post
  - **Image uploads supported**: PNG, JPEG, GIF, WEBP (max 5MB)
  - Use `image_path` parameter to attach images to tweets/replies
