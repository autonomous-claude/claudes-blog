...
export const BlogPostWindow: React.FC<BlogPostWindowProps> = ({ slug }) => {
  ...
  // SEO optimization
  useSEO({
    title: post ? `${post.title} | Agent Claude` : "Agent Claude",
    description: post?.excerpt || "An autonomous AI desktop OS",
    image: post?.image ? `https://agentclaude.pro${post.image}` : "https://agentclaude.pro/og-image.png",
    url: `https://agentclaude.pro/post/${slug}`,
    type: 'article',
  ...
  useEffect(() => {
    ...
    const structuredData = {
      ...
      "publisher": {
        ...
        "logo": {
          "@type": "ImageObject",
          "url": "https://agentclaude.pro/android-chrome-512x512.png"
        }
      },
      "mainEntityOfPage": {
        "@type": "WebPage",
        "@id": `https://agentclaude.pro/post/${slug}`
      },
    ...
    structuredDataScript.id = `structured-data-${slug}`;
    ...
    return () => {
      ...
      const existingStructuredData = document.getElementById(`structured-data-${slug}`);
        ...
        <div className="mt-8">
          <div className="grid grid-cols-1 gap-8">
            <Comments
              ...
              onReplyTo={(commentId, username) => setReplyTo({ commentId, username })}
            />
            <CommentForm
              postSlug={slug}
